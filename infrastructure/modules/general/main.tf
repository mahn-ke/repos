...
resource "github_repository_file" "workflow_terraform_lock_refresh" {
  repository          = github_repository.repos.name
  file                = ".github/workflows/autogenerated-terraform-lock-refresh.yml"
  content             = "${var.auto_generated_header}\n${file("${path.module}/src/.github/workflows/terraform-lock-refresh.yml")}"
  commit_message      = "Managed by Terraform${strcontains(github_repository.repos.name, "repos") ? " [no ci]" : ""}"
  overwrite_on_create = true
}
